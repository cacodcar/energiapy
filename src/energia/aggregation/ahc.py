...
from ..components.temporal import TemporalScale
...
def ahc(
    ...
    #         [pandas.DataFrame(capacity_factor[i]) for i in capacity_factor.keys()],
    #         axis=1,
    #     )
    ...
    #     [price_factor_df, capacity_factor_df, demand_factor_df], axis=1
    ...
    reduced_scenario_scaleiter = numpy.array_split(
        reduced_temporal_scale.scale_iter(scale_level=scale_level + 1),
        len(parent_scale),
    )
    for parent_iter in range(len(parent_scale)):
        ...
        if len(combined_df) == len(parent_scale) * len(scale):
            ...
            scaled_df = scaler(
                input_df=split_df[parent_iter], scale=scale, child_scale=child_scale
            )
        ...
        connectivity_matrix = generate_connectivity_matrix(scale_len=len(scale))
        ...
        nearest_centroid_array = (
            NearestCentroid().fit(scaled_array, clustered_array).centroids_
        ...
        for cluster_no in nearest_centroid_df['cluster_no']:
            ...
            cluster_array = (
                scaled_df[scaled_df['cluster_no'] == cluster_no].iloc[:, :-1].to_numpy()
            )
            centroid_array = (
                nearest_centroid_df[nearest_centroid_df['cluster_no'] == cluster_no]
                .iloc[:, :-1]
                .to_numpy()
            )
            ...
            for cluster in cluster_array:
                ...
                euclidean_distance = find_euclidean_distance(cluster_point, centroid)
        ...
        for i in closest_to_centroid_df['period']:
            list_.append(
                scaled_df.index.values[
                    scaled_df['ED']
                    == scaled_df['ED'][scaled_df['cluster_no'] == i].min()
                ][0]
            )
        # find the actual day closest to centroid
        ...
        output_dict = {
            scale: {
                'rep_period': (
                    parent_iter,
                    closest_to_centroid_df['rep_period'][
                        closest_to_centroid_df['period'] == scale[scale_level]
                    ].values[0],
                    *scale[scale_level + 1 :],
                ),
                'cluster_wt': closest_to_centroid_df['cluster_wt'][
                    closest_to_centroid_df['period'] == scale[scale_level]
                ].values[0],
            }
...
#     capacity_factor: dict = None,
#     demand_factor: dict = None,
# ):
#     """perform agglomerative hierarchial clustering over time-series data
...
#     wcss_list = []
#     for i in iter_:
#         rep_dict_iter, reduced_temporal_scale, info_dict = agg_hierarchial(
...
#         y_line = [
#             theta[2] + theta[1] * pow(x, 1) + theta[0] * pow(x, 2) for x in range_list
#         ]
